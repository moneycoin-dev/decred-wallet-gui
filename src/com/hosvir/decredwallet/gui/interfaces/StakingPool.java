...
public class StakingPool extends Interface {
	...
	public void init() {		
		...
		this.components.add(new Label("importlabel", Constants.getLangValue("Import-Script-Label"), 40, 240));
		...
		this.components.add(new Button("getpublickey", Constants.getLangValue("Get-Public-Key-Button-Text"), Engine.getWidth() - 240, 168, 200, 35, Constants.flatBlue, Constants.flatBlueHover));
		this.components.add(new Button("import", Constants.getLangValue("Import-Button-Text"), Engine.getWidth() - 140, 218, 100, 35, Constants.flatBlue, Constants.flatBlueHover));
	...
	public void update(long delta) {
		...
		if(importReady && Constants.isPrivatePassphraseSet()) {
			...
			getComponentByName("errordiag").text = "Imported script";
