...
public class SettingsSecurity extends Interface {
	...
	public void init() {
		...
		Button dumpButton = new Button("dumpWallet", "Dump wallet", Engine.getWidth() - 190, Engine.getHeight() - 105, 150, 35, Constants.flatBlue, Constants.flatBlueHover);
		...
		this.components.add(new Button("dumpPrivateKey", Constants.getLangValue("Dump-Private-Key-Button-Text"), Engine.getWidth() - 240, 170, 200, 35, Constants.flatBlue, Constants.flatBlueHover));
		this.components.add(new Button("changePassphrase", Constants.getLangValue("Change-Passphrase-Button-Text"), Engine.getWidth() - 240, 410, 200, 35, Constants.flatBlue, Constants.flatBlueHover));
		this.components.add(new Button("getMasterPublicKey", Constants.getLangValue("Get-Master-Public-Key-Button-Text"), 40, Engine.getHeight() - 105, 220, 35, Constants.flatBlue, Constants.flatBlueHover));
		...
		if(!blockInput) {
			...
			if(Constants.isPrivatePassphraseSet() && getComponentByName("addressInput").text != "") {
				...
				if(unlock == null | unlock.trim().length() < 1){
					String address = Api.dumpPrivKey(getComponentByName("addressInput").text);
					...
					if(c.selectedId == 0 && c.enabled){
						...
						case "changePassphrase":
							if(!getComponentByName("newPassphraseInput").text.contains(getComponentByName("confirmPassphraseInput").text)){
								...
								String changed = Api.wallPassphraseChange(getComponentByName("oldPassphraseInput").text, getComponentByName("newPassphraseInput").text);
